...
- **Firestore**: §4.9 컬렉션. **규칙 정본 = `firestore.rules`(git, 3앱 통합본 — DL-033).** 규칙 변경은 이 파일만 수정 후 `firebase deploy --only firestore:rules`(호두님 직접). **규칙 배포는 이 repo 하나에서만** — 다른 repo(운영·생산·재고)는 `firebase.json`에서 firestore 타깃 제거(DL-036). 신규 컬렉션 규칙도 이 파일에만.
